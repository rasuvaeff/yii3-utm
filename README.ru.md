...
Cookie переписывается **только когда история изменилась** — именно это
оставляет неизменённый ответ без `Set-Cookie` и кэшируемым, и именно из-за
этого `ttlDays` отсчитывается от последнего *касания*, а не от последнего
*визита*. Посетитель, который каждый день заходит по прямой ссылке, нового
`Set-Cookie` не получает, поэтому окно атрибуции закрывается через 30 дней
после последнего касания кампании, хотя человек никуда не уходил. Обновление
cookie на обычном визите потребовало бы поля «когда записано» в её payload и
добавило бы `Set-Cookie` к ответам, которые сегодня кэшируются; если нужно
окно длиннее — увеличьте `ttlDays`.

| Опция | По умолчанию | Эффект |
|---|---|---|
| `enabled` | `true` | Общий выключатель |
| `ignoredPaths` | `[]` | Пути, которые пропускаются; сравнение по границе сегмента: `/api` пропускает `/api` и `/api/v1`, но не `/api-docs` |
...
| Landing page | Обрезается до 500 символов по границе, которая оставляет значение URL; санитайзинг query выполняется до хранения |
| Cookie | При декодировании санируется ровно как query string, а её размер считается после percent-encoding |

**Экранирование — обязанность потребителя.** Нормализация снимает управляющие
символы, обрезает пробелы и длину — она не делает значение безопасным для
вывода. `?utm_source=<img src=x onerror=alert(1)>` доедет до cookie, атрибутов
запроса и журнала атрибуции ровно этим текстом: библиотека, которая ничего не
рендерит, не знает, в какой контекст (HTML, атрибут, JSON, ячейка CSV) значение
попадёт. Экранируйте на выводе — типичное место, где это важно, маркетинговый
дашборд со списком названий кампаний.
